...
'''
    ...
    # Execute the patch script on remote server with virtual environment
    cmd = f"ssh root@170.64.199.151 'cd /root/test && source dashboard_venv/bin/activate && python3 -c \"import time\\n{patch_script}\"'"
...
def verify_fix():
    ...
    check_cmd = "ssh root@170.64.199.151 'cd /root/test && source dashboard_venv/bin/activate && grep -n \"_save_predictions_to_table\" enhanced_ml_system/analyzers/enhanced_morning_analyzer_with_ml.py'"
    ...
    count_cmd = "ssh root@170.64.199.151 'cd /root/test && source dashboard_venv/bin/activate && sqlite3 data/trading_predictions.db \"SELECT COUNT(*) FROM predictions;\"'"
...
def test_morning_routine():
    ...
    cmd = "ssh root@170.64.199.151 'cd /root/test && source dashboard_venv/bin/activate && export PYTHONPATH=/root/test && timeout 300 python3 -m app.main morning'"
    result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    ...
    if result.returncode == 0:
        ...
        if int(count) > 0:
            ...
            sample_cmd = "ssh root@170.64.199.151 'cd /root/test && source dashboard_venv/bin/activate && sqlite3 data/trading_predictions.db \"SELECT symbol, predicted_action, action_confidence, prediction_timestamp FROM predictions ORDER BY prediction_timestamp DESC LIMIT 5;\"'"
