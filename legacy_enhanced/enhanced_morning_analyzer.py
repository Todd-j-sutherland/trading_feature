...
def run_continuous_analysis():
    """Run analysis every 30 minutes during market hours"""
    import time
    from datetime import datetime, time as dt_time
    
    print("🚀 Starting continuous morning analyzer - runs every 30 minutes during market hours")
    ...
    while True:
        ...
        if current_time.weekday() < 5:  # Monday to Friday
            # Check if it's during market hours (9 AM to 4 PM)
            market_start = dt_time(9, 0)   # 9:00 AM
            market_end = dt_time(16, 0)    # 4:00 PM
            current_time_only = current_time.time()
            
            if market_start <= current_time_only <= market_end:
                print(f"\n⏰ {current_time.strftime('%Y-%m-%d %H:%M:%S')} - Running scheduled analysis...")
                try:
                    main()  # Run the analysis
                    print(f"✅ Analysis completed successfully at {datetime.now().strftime('%H:%M:%S')}")
                except Exception as e:
                    print(f"❌ Analysis failed: {e}")
                
                # Wait 30 minutes (1800 seconds)
                print("⏳ Waiting 30 minutes until next analysis...")
                time.sleep(1800)
            else:
                # Outside market hours - check every hour
                print(f"💤 Outside market hours ({current_time.strftime('%H:%M')}). Next check in 1 hour...")
                time.sleep(3600)
        else:
            # Weekend - check every 4 hours
            print(f"📅 Weekend ({current_time.strftime('%A')}). Next check in 4 hours...")
            time.sleep(14400)
