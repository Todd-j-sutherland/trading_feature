...
try:
    ...
    print("✅ IG Markets integration available for main prediction system")
except ImportError:
    IG_INTEGRATION_AVAILABLE = False
    print("⚠️ IG Markets integration not available - using yfinance only")
        ...
        if IG_INTEGRATION_AVAILABLE:
            try:
                ...
                if self.ig_markets_enabled:
                    ...
                    print("✅ IG Markets enabled for market context analysis")
                else:
                    print("⚠️ IG Markets initialization failed - using yfinance for market context")
            except Exception as e:
                print(f"❌ IG Markets integration error: {e}")
                self.ig_markets_enabled = False
    
    def get_current_price(self, symbol):
        """Get current price with IG Markets fallback to yfinance"""
        if self.ig_markets_enabled and self.enhanced_price_source:
            try:
                ...
                print(f"IG Markets price fetch failed for {symbol}: {e}")
