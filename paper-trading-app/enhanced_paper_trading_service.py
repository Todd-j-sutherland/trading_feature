...
class EnhancedPaperTradingService:
    ...
    def check_position_exits(self):
        ...
        for symbol, position in self.active_positions.items():
            ...
            # Max hold time exceeded
                ...
                should_exit = True
                exit_reason = f"Max hold time exceeded ({hold_time_minutes:.0f} >= {self.config['max_hold_time_minutes']} min)"
            
            if should_exit:
                positions_to_close.append((symbol, current_price, current_profit, exit_commission, exit_reason, hold_time_minutes))
            else:
                # Log current status
                logger.info(f"📊 {symbol}: ${current_price:.2f} (P&L: ${current_profit:.2f}, Hold: {hold_time_minutes:.0f}min)")
    ...
    def close_position(self, symbol: str, exit_price: float, profit: float, exit_commission: float, exit_reason: str, hold_time_minutes: float):
        ...
        try:
            ...
            logger.info(f"   P&L: ${profit:.2f} | Hold: {hold_time_minutes:.0f}min")
    ...
    def run(self):
        ...
        while self.running:
            try:
                ...
                # Check active positions every minute
                if self.active_positions:
                ...
                if current_time - last_prediction_check >= self.config['prediction_check_interval_seconds']:
