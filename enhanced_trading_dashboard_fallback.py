    ...
    <script>
        ...
        window.addEventListener('load', function() {
            if (typeof LightweightCharts === 'undefined') {
                ...
                fallbackScript.src = 'https://cdn.jsdelivr.net/npm/lightweight-charts/dist/lightweight-charts.standalone.production.js';
                ...
                fallbackScript.onerror = function() {
                    console.error('Both CDNs failed to load TradingView library');
                    document.getElementById('loading').textContent = 'Chart library failed to load from all sources. Please check your internet connection.';
                };
...
<body>
    ...
    <script>
        ...
        function initChart() {
            ...
            if (typeof LightweightCharts === 'undefined') {
                ...
                document.getElementById('loading').textContent = 'Chart library not loaded. Please refresh the page.';
                return;
            }
            ...
            try {
                ...
                document.getElementById('loading').textContent = 'Error initializing chart: ' + error.message;
            }
        }
        ...
        async function loadChartData() {
            try {
                ...
                if (data.candlesticks && data.candlesticks.length > 0) {
                    ...
                    if (!volumeSeries && legendState.volume) {
                    ...
                    if (!sma20Series && legendState.sma20) {
                    ...
                    if (!sma50Series && legendState.sma50) {
                    ...
                    const techData = generateTechnicalAnalysis(data.candlesticks);
                    
                    // Update series based on legend state
                    if (volumeSeries) {
                        volumeSeries.setData(legendState.volume ? techData.volume : []);
                    }
                    if (sma20Series) {
                        sma20Series.setData(legendState.sma20 ? techData.sma20 : []);
                    }
                    if (sma50Series) {
                        sma50Series.setData(legendState.sma50 ? techData.sma50 : []);
                    }
        ...
        function generateTechnicalAnalysis(candlesticks) {
            const volume = [];
            const sma20 = [];
            const sma50 = [];
            
            // Generate volume data (mock for now)
            candlesticks.forEach((candle, index) => {
                volume.push({
                    time: candle.time,
                    value: Math.random() * 1000000 + 500000 // Mock volume
                });
            });
        ...
        document.addEventListener('DOMContentLoaded', () => {
            // Add a small delay to ensure TradingView library is fully loaded
            setTimeout(() => {
                initChart();
                loadChartData();
            }, 100);
    ...
    def get_mock_price_data(self, symbol, hours):
        ...
        # Determine interval based on timeframe to get proper data density
        if hours <= 0.5:        # 30 minutes
            interval_minutes = 1    # 1-minute intervals = ~30 points
        elif hours <= 4:        # 4 hours  
            interval_minutes = 5    # 5-minute intervals = ~48 points
        elif hours <= 24:       # 1 day
            interval_minutes = 15   # 15-minute intervals = ~96 points
        elif hours <= 72:       # 3 days
            interval_minutes = 60   # 1-hour intervals = ~72 points
        else:                   # 7+ days
            interval_minutes = 60   # 1-hour intervals = ~168 points for 7 days
        ...
        # Add some realistic trending behavior
        trend_direction = 1 if random.random() > 0.5 else -1
        trend_strength = random.uniform(0.001, 0.003)
        
        while current_time <= now:
            # Add trending movement with some noise
            trend_move = trend_direction * trend_strength * interval_minutes
            noise = random.uniform(-0.5, 0.5)
            price_change = trend_move + noise
            
            current_price += price_change
            current_price = max(80, min(120, current_price))  # Keep within realistic bounds
        ...
        logger.info(f"Generated {len(candlesticks)} mock data points for {hours} hours with {interval_minutes}min intervals")
        return candlesticks
